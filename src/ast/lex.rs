...
use self::{LiteralKind::*, TokenKind::*};
...
impl Default for Token {
    fn default() -> Self {
        Self { kind: TokenKind::Unknown, len: 0 }
    }
}
impl Token {
    crate fn new(kind: TokenKind, len: usize) -> Token {
        Token { kind, len }
    }
}
...
pub enum RawStrError {
    ...
    /// The string was never terminated. `possible_terminator_offset` is the number of characters
    /// after `r` or `br` where they may have intended to terminate it.
    ...
    fn advance_token(&mut self) -> Token {
        ...
        let token_kind = match first_char {
            ...
            'r' => match (self.first(), self.second()) {
                // This was Raw ident as in #ident now it errors
                ('#', c1) if is_id_start(c1) => Unknown,
                ('#', _) | ('"', _) => {
