			...
			Global::Array { name, content } => {
				...
				let mut global = if content.is_empty() {
					...
					let content_str = content.iter().map(|v| {
						match v {
							inst::Val::Integer(i) => format!(".int {}", i),
							inst::Val::UInteger(u) => format!(".uint {}", u),
							inst::Val::Float(f) => format!(".float {}", f),
							inst::Val::Location(loc) => unreachable!("is this posible {}", loc),
							inst::Val::String(c) => format!(".string {}", c),
							inst::Val::Null => ".zeros 4".to_string(),
						}
					}).collect::<Vec<_>>().join(",\n	");
				...
				write!(buf, "	.global {}, {}, 4{}", name, size, global);
			}
